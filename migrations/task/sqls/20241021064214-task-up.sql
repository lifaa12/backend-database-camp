...
(
(select "USER".id from "USER" where "USER".name = '王小明'),
(select "COURSE".id from "USER" join "COURSE" on "USER".id = "COURSE".user_id where "USER".name = '李燕容'),
'2024-11-24 16:00:00',
'即將授課'
);
    -- 2. 新增： `好野人` 預約 `李燕容` 的課程
...
insert into "COURSE_BOOKING" (user_id,course_id,booking_at,status) values
(
(select "USER".id from "USER" where "USER".name = '好野人'),
(select "COURSE".id from "USER" join "COURSE" on "USER".id = "COURSE".user_id where "USER".name = '李燕容'),
'2024-11-24 16:00:00',
'即將授課'
);
...
select 
  "USER".id as user_id,
  name as 用戶名稱,
  (select sum(purchased_credits) from "CREDIT_PURCHASE" where "CREDIT_PURCHASE".user_id = "USER".id)-
  (select count(join_at) from "COURSE_BOOKING" where "COURSE_BOOKING".user_id = "USER".id) as remaining_credit
from "USER"
where "USER".name = '王小明';
...
select
  "CREDIT_PACKAGE".name as 方案名稱,
  count(credit_package_id) as 銷售數量
from "CREDIT_PURCHASE"
...
select 
 ...
 from "COURSE_BOOKING"
 where status = '即將授課' and extract (month from created_at)=11;